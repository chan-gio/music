...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "Update songs set sname ='$sname', sartist = '$sartist' where sid = $sid";
    ...
    if ($_FILES['txtsimage']['size'] > 0) {
        ...
        if (move_uploaded_file($_FILES["txtsimage"]["tmp_name"], $target_file)) {
            $sql = "Update songs set simg = '$new_image_name' where sid = $sid";
            $conn->query($sql) or die($conn->error);
        } else {
    ...
    if ($_FILES['txtssrc']['size'] > 0) {
        ...
        if (move_uploaded_file($_FILES["txtssrc"]["tmp_name"], $target_file)) {
            $sql = "Update songs set ssrc = '$new_audio_name' where sid = $sid";
            $conn->query($sql) or die($conn->error);
        } else {
